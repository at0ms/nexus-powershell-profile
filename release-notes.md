# PowerShell Profile Release Notes

### 1.0.5 - February 13, 2026
* Added inital shortcuts (`edit`, `omp`). 
* Added new command `prss`.
* Added profile settings support `profile.ps1`. Allows the user to override certain settings.
* Refactored code and comments

### 1.0.4 - February 11, 2026
* Show-Host & Show-Information now uses these helper functions `Write-EmptyLine`, `Write-Header`, `Write-Section` and `Write-SectionOption`.
* Refactored code and comments

### 1.0.3 - February 10, 2026
* Added command caching. Credit to DreamTimeZ's Powershell profile
* Refactored code and comments

### 1.0.2 - February 10, 2026
* Added an optional message when a new PowerShell session is started (disabled by default)
* Refactored some of the code.

### 1.0.1 - February 10, 2026
* Changed script version from one decimal point to two `1.0 > 1.0.0` for smaller releases.
* Added links to the git repo and release notes to `Show-Information` function.
* Refactored some of the code.

### 1.0 - February 10, 2026
Initial release